                ...
                with col_b:
                    action = st.selectbox("Action", 
                                        ["🔒 Lock Account", "🔓 Unlock Account", 
                                         "🔄 Reset Attempts", "🗑️ Delete Account"])
...
# =========================
    ...
    if media == "Video":
        with st.expander("📹 **Why MP4 output is not supported?**", expanded=False):
            st.markdown("""
            ...
            **This system uses:** Simple LSB (Least Significant Bit) steganography
            
            **Output will be AVI format** - it preserves your hidden message perfectly
            """)
    ...
    if up and key:
        ...
        try:
            ...
            if mode == "Embed Message":
                # Warn about AVI output for video
                if media == "Video" and up.name.lower().endswith('.mp4'):
                    st.warning("⚠️ **Output will be AVI format** - MP4 compression destroys hidden data. "
                               "Your embedded video will be saved as .avi file to protect your secret message.")
                ...
                if st.button("🔓 Extract & Decrypt"):
                    try:
                        if media == "Image":
                            ...
                            # Extract from video (AVI or MP4 input, but extraction uses same method)
